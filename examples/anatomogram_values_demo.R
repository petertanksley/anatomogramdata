# Demo: the quick-start path via anatomogram_select(organs = ...) -- skips
...
# together).
#
# anatomogram_select() resolves organ names (fuzzy, case-insensitive partial
# match against tissue_name -- see anatomogram_select("male") for the browse
...
p <- ggplot(d, aes(x, y, group = group)) +
  ...
  coord_fixed() +
  theme_void() +
  labs(title = "anatomogram_select(organs = ...) quick-start: one value per organ, geom_polygon() only",
       fill = "Value")

ggsave("examples/anatomogram_values_demo.png", p, width = 6, height = 8, dpi = 150)
cat("Wrote examples/anatomogram_values_demo.png\n")
