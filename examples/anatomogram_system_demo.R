# Demo: filtering/faceting by organ system via anatomogram_select(system = ...),
...
# the outline_role column directly on hgMale, unrelated to anatomogram_select()
...
ggsave("examples/anatomogram_system_silhouette_vs_contour.png", p1, width = 6, height = 8, dpi = 150)

# (b) "all bones" -- one system, no values needed. anatomogram_select()'s
...
p2 <- ggplot(bones, aes(x, y, group = group)) +
  ...
  expand_limits(x = outline$x, y = outline$y) +
  coord_fixed() + theme_void() +
  labs(title = "anatomogram_select(\"male\", system = \"Skeletal\")")
ggsave("examples/anatomogram_system_bones.png", p2, width = 6, height = 8, dpi = 150)
...
several <- anatomogram_select("male", system = c("Skeletal", "Digestive", "Respiratory", "Endocrine"))
p3 <- ggplot(several, aes(x, y, group = group)) +
  ...
  coord_fixed() + theme_void() +
  facet_wrap(~ system) +
  labs(title = "anatomogram_select(\"male\", system = c(\"Skeletal\",\"Digestive\",\"Respiratory\",\"Endocrine\"))")
ggsave("examples/anatomogram_system_facets.png", p3, width = 10, height = 10, dpi = 150)

cat("Wrote examples/anatomogram_system_silhouette_vs_contour.png, _bones.png, _facets.png\n")
